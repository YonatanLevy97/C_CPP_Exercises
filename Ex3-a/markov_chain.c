...
void free_markov_chain(MarkovChain ** ptr_chain)
{
    ...
    for(int i = 0; nodes_traveler != NULL; i++)
    {
...
MarkovNode* get_first_random_node(MarkovChain *markov_chain)
{
    ...
    while (traveler != NULL && traveler->data != NULL &&
            traveler->data->data[strlen(traveler->data->data)-1] == '.');
...
bool add_node_to_counter_list(MarkovNode *first_node, MarkovNode *second_node)
{
    ...
    int index = is_node_in_counter_list(first_node,second_node);
    if (index != -1 && first_node->counter_list != NULL)
    {
    ...
    {
        first_node->size++;
        first_node->counter_list[(first_node->size)-1] = (NextNodeCounter){second_node,1};
        return true;
    }
    return false;
}
...
Node* add_to_database(MarkovChain *markov_chain, char *data_ptr)
{
    ...
    {
        MarkovNode* markov_node_word = malloc(sizeof (*markov_node_word));
        if (markov_node_word == NULL)
        {
            return NULL;
        }
        markov_node_word->data = malloc(strlen(data_ptr)+1);
        if(markov_node_word->data == NULL)
        {
            return NULL;
        }
        memcpy(markov_node_word->data , data_ptr, strlen(data_ptr)+1);
        markov_node_word->counter_list = NULL;
        markov_node_word->size = 0;
                                            ...
                                            first_node, int max_length)
{
    ...
    int tweets_num = 2;
    printf(" %s", next_word->data);
    while(tweets_num < max_length &&
            next_word->data[strlen(next_word->data)-1] !='.')
    {
